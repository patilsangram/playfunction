...
playfunction.selling.set_field_permissions = function(doc) {
	...
	if(has_common(frappe.user_roles, ["Playfunction Customer"]) && frappe.session.user != 'Administrator') {
		...
		var common_form_fields = {
			"read_only":["customer_name", "transaction_date", "company","valid_till", "order_type", "customer_address", "contact_person","shipping_address_name", "territory", "apply_discount_on"],
			"hidden": ["terms_section_break", "shipping_rule","taxes_section","taxes","currency_and_price_list","subscription_section"]
		}
